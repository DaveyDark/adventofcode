...
fn simulate_beam(grid: &Vec<Vec<char>>, beam: Beam) -> u64 {
...
pub fn solve(input: String) -> u64 {
    ...
    for i in 0..grid.len() as i32 {
        max_tiles = max_tiles.max(simulate_beam(
            &grid,
            Beam::new_with_dir(i, -1, Direction::East),
        ));
        max_tiles = max_tiles.max(simulate_beam(
            &grid,
            Beam::new_with_dir(-1, i, Direction::South),
        ));
        max_tiles = max_tiles.max(simulate_beam(
            &grid,
            Beam::new_with_dir(i, grid.len() as i32, Direction::West),
        ));
        max_tiles = max_tiles.max(simulate_beam(
            &grid,
            Beam::new_with_dir(grid.len() as i32, i, Direction::North),
        ));
    }
    max_tiles
}

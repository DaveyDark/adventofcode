...
fn count_differences(s1: &str, s2: &str) -> u64 {
    s1.chars()
        .zip(s2.chars())
        .fold(0, |acc, (c1, c2)| acc + (c1 != c2) as u64)
}
...
fn check_row_symmetry(grid: &[&str], weight: u64) -> u64 {
    ...
    for i in 1..grid.len() {
        ...
        let orig = grid[i..i + size]
            .iter()
            .fold(String::new(), |acc, x| acc + x);

        // Instead of checking strict equality, we need to check that there is exactly one
        // difference between the strings
        if mirror.len() == orig.len() && count_differences(&mirror, &orig) == 1 {
            sum += weight * i as u64;
        }
    }
    sum
